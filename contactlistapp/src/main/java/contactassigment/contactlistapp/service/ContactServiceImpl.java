...
public class ContactServiceImpl implements ContactService {
  ...
  public List<ContactDTO> listByCriteriaFetchOrganisation(ContactSearchCriteriaDTO criteria) {
    ...
    List<Contact> resultList = contactRepo.searchByNamesFetchOrganisation(criteria.getFirstName(),criteria.getLastName(), criteria.getOrganisationName());
  ...
  public ContactDTO updateByDTO(ContactDTO contactDTO) throws EntityNotFoundException {
    ...
    if (Integer.valueOf("-1").equals(contactDTO.getOrganisation().getId())) {
      ...
      if (!persistedOrg.isPresent()) {
        throw new EntityNotFoundException(String.format("Unable to find Entity: %s with id: %d", Organisation.class.getCanonicalName(), contactDTO.getOrganisation().getId()));
      }
